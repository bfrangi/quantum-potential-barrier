...
def plot_T_vs_E(E, T):
    ...
    if len(E) == 1:
        ax.scatter(E, T, c='orange', alpha=0.5)
    else:
    ...
    if DE:
        E_list = [min(E) + i * DE for i in range(10000)]
    else:
        E_list = [E[0]]
    ...
    if len(E_list) == 1:
        ax.scatter(E_list, T_list, c='blue', alpha=0.5)
    else:
...
def choice_1():
    ...
    def animate(i):
        ...
        if integral_list:
            ...
            T_prob.plot(t[0:i*10], integral_list[0:i*10],
...
def choice_5(k_0_list):
    ...
    k_0_list_custom = parse_k0_list(
        input("Press [enter] for default, " +
              str(k_0_list) + f":{bcolors.OKGREEN} ")
    )
    ...
    for k_0_val in k_0_list:
        ...
        wf_modulus_squared = main_from_scratch(
            export_and_integrate=False, k_0=k_0_val)
