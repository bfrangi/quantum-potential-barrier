...
def initial_wf(x): return (pi * sigma_0**2)**(-1/4) * \
...
if __name__ == "__main__":
    ...
    if N_custom and not N_custom.isdigit(): # If the user has specified a non-integer N_custom, stop execution
        print(f"{bcolors.FAIL}Invalid Input{bcolors.ENDC}")
        exit()
    elif N_custom: # If the user has specified an integer N_custom, accept it. If N_custom = '', do nothing, so the default N remains
        N = int(N_custom)
    ...
    if not skipped and choice == "1":
        ...
        if N >= 2000:
            ...
            print(
                f"{bcolors.FAIL}Cannot make this computation: N (value: {N}) is smaller than 2000{bcolors.ENDC}")
    elif not skipped and choice == "3":
        print("The accuracy of this method will be analysed by computing the integral of |ψ|2 ".translate(
            SUP) + "and calculating its average and deviation from the theroetical value, which is 1.")
        ...
        print("Standard deviation of ∫ |ψ|2 dx with respect to one =".translate(
            SUP) + f"{bcolors.OKCYAN}", sd, f"{bcolors.ENDC}")
    elif not skipped and choice == "4":
        integral_list = transmission_prob(wf_modulus_squared)

        t = [k*Dt for k in range(N)]
        fig, ax = plt.subplots()
        plt.axhline(y=integral_list[-1], color='black',
                    linestyle='dashed', label=f"T = {integral_list[-1]}")
        ...
        print("Enter the values of k0".translate(
            SUB) + " (separated by commas) for which you would like to calculate the transmission probability.")
        k_0_list_custom = input(
            "Press [enter] for default, " + str(k_0_list) + f":{bcolors.OKGREEN} ")
        if k_0_list_custom:
        ...
        for i in k_0_list:
            ...
            print(f"{bcolors.UNDERLINE}Making Calculations for:{bcolors.ENDC}{bcolors.OKBLUE}",
                  "K0 =".translate(SUB), str(k_0) + f"{bcolors.ENDC}")
            wf_modulus_squared = main_from_scratch(export_and_integrate=False)
            transmission_probability_functions.append(
                transmission_prob(wf_modulus_squared))
            print("")
        ...
        plt.savefig('Output-Media/probability_of_transmission_comparison_sigma_' +
                    re.sub(r'\.', r'\,', str(sigma_0)) + '.pdf', bbox_inches='tight')
        f = os.path.dirname(os.path.realpath(
            __file__)) + "/Output-Media/probability_of_transmission_comparison_sigma_" + re.sub(r'\.', r'\,', str(sigma_0)) + ".pdf"
        print("Saved figure to", f)

        plt.show()
        choice = input(
            "Do you want to study the dependence of the (asymptotic) transmission probability on the energy E0? [Y/n]".translate(SUB) + f" {bcolors.OKGREEN}")
        print(f"{bcolors.ENDC}")
        if choice.lower() in ["yes", "y"]:
            E_0_list = [(k**2 + 1 / (2 * sigma_0**2)) / 2 for k in k_0_list]
            asymptotic_T = [detect_asympote(
                integral_list) for integral_list in transmission_probability_functions]
